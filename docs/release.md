...
SOFTLOAF_TRICHROME_VERSION="0.2.0-beta.3" \
...
SOFTLOAF_NOTARY_KEYCHAIN_PROFILE=softloaf-notary \
  tools/notarize_macos_dmg.sh build-release/SoftLoaf-Trichrome-0.2.0-beta.3-macOS.dmg
```
...
beta releases, match the structure while only publishing supported artifacts.
...
The current workflow names are still accepted for `v0.2.0-beta.3`, but new
...
```bash
git add docs/release_notes/v0.2.0-beta.3.md
git commit -m "Add v0.2.0-beta.3 release notes"
git tag -a v0.2.0-beta.3 -m "SoftLoaf Trichrome v0.2.0-beta.3"
...
```bash
gh release create v0.2.0-beta.3 \
  build-release/SoftLoaf-Trichrome-0.2.0-beta.3-macOS.dmg \
  build-release/SoftLoaf-Trichrome-0.2.0-beta.3-macOS.dmg.sha256 \
  --title "SoftLoaf Trichrome v0.2.0-beta.3" \
  --notes-file docs/release_notes/v0.2.0-beta.3.md \
  --draft \
  --prerelease
```
...
1. create or verify the annotated tag, for example `v0.2.0-beta.3`;
...
1. For `v0.2.0-beta.3`, publish Windows only as explicit unsigned tester
   artifacts.
...
gh release view v0.2.0-beta.3 --json isDraft,isPrerelease,assets
```

For the earlier `v0.1.0-alpha.1`, the successful Windows run was `28321855649`.
The important fixed decisions from that run are:
...
- An unsigned Windows beta installer is expected to trigger SmartScreen.
